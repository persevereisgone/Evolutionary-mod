...
$res = powershell -NoProfile -ExecutionPolicy Bypass -File $s -Method $Method -ParamsFile $ParamsFile 2>&1 | Out-String
...
if ($m.Success) {
    ...
    Write-Host "No image data found. Raw:"
    Write-Host $res.Substring(0, [Math]::Min(800, $res.Length))
}